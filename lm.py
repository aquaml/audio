...
class LMModel(StreamingModule):
    ...
    def __init__(self, pattern_provider: CodebooksPatternProvider, condition_provider: ConditioningProvider,
        ...
        self.informer = aqua.static.static_informer("localhost", 8080, 0)
        self.have_informed: bool = False
    ...
    def generate(self,
        ...
        previous_log_time = self.get_time_s()
        free_mem = 100
        ...
        with self.streaming():
            ...
            for offset in range(start_offset_sequence, gen_sequence_len):
                ...
                if self.get_time_s() - previous_log_time > 1:
                    previous_log_time = self.get_time_s()
                torch.cuda.empty_cache()
                curr_free_mem = torch.cuda.get_device_properties(0).total_memory - torch.cuda.memory_reserved(0)                

                curr_free_mem /= (1024 ** 3)
                if curr_free_mem < free_mem:
                    free_mem = curr_free_mem
                
        unconditional_state.clear()
        print("Free memory: {}".format(free_mem))
        if not self.have_informed:
            self.informer.offer_memory(free_mem * (1024 ** 3))
            self.have_informed = True
